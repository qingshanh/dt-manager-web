...
test("direct SMS parser trims trailing push metadata from plaintext content", () => {
  const json = Buffer.from(JSON.stringify({ k1: 561, k2: "(202) 555-0101", k3: "meta-regression" }), "utf8");
...
test("direct SMS parser does not use the sender number as the target phone", () => {
  const metadata = Buffer.from("sender=12025550101 target=33199000001", "latin1").toString("base64");
  const json = Buffer.from(JSON.stringify({ k1: 561, k2: "(202) 555-0101", info: metadata, k3: "target-regression" }), "utf8");
  ...
  assert.equal(parsed?.fromNumber, "(202) 555-0101");
...
test("direct SMS parser leaves target empty when metadata only repeats the sender", () => {
  const metadata = Buffer.from("sender=12025550101", "latin1").toString("base64");
  const json = Buffer.from(JSON.stringify({ k1: 561, k2: "(202) 555-0101", info: metadata, k3: "sender-only-regression" }), "utf8");
