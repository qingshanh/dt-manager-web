...
test("release version stays synchronized across production entry points", () => {
  ...
  assert.match(read("backend/src/config.ts"), /APP_VERSION: z\.string\(\)\.default\("0\.2\.9"\)/);
  assert.match(read("frontend/vite.config.ts"), /'0\.2\.9'/);
  assert.match(read("frontend/Dockerfile"), /ARG APP_VERSION=0\.2\.9/);
  assert.match(read("docker-compose.yml"), /APP_VERSION: \$\{APP_VERSION:-0\.2\.9\}/);
  assert.match(read("docker-compose.yml"), /VITE_APP_VERSION: \$\{VITE_APP_VERSION:-0\.2\.9\}/);
  assert.match(read("start.ps1"), /APP_VERSION" -Default "0\.2\.9"/);
  assert.match(read(".env.example"), /APP_VERSION=0\.2\.9/);
  assert.match(read(".env.example"), /VITE_APP_VERSION=0\.2\.9/);
  assert.match(read("deploy/systemd/dt-manager.env.example"), /APP_VERSION=0\.2\.9/);
});
